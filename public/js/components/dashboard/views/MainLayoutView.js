...
define(['marionette'], function(Marionette, Usuarios) {
  ...
  MainLayout = (function() {
    class MainLayout extends Marionette.LayoutView {
      onRender() {
        var ano_fim, ano_inicio, display, j, len, mes_fim, mes_inicio, meses, o, ref;
        ref = window.modulo.ofertas;
      ...
      buscar() {
        ...
        App.socket.emit('dashboard.surtos.g9', query, function(resposta) {
          ...
          return App.execute('desenha_grafico_surtos', 'grafico-qual-profissao', 'Profissão ou o que está estudando', [
            ...top5,
            {
              name: "Outro",
              y: valor_outro
        ...
        App.socket.emit('dashboard.surtos.g10', query, function(resposta) {
          var _data, data, entry, j, k, l, len, len1, len2, n, ordem, prop, ref, ref1, ref2, v;
          if (!resposta.ok) {
            return;
          }
    ...
    App.commands.setHandler('desenha_grafico_surtos', function(el, titulo, data, colors = ["#1961ac", "#3c9687", "#9cd3c8", "#51b8e5", "#4dd6bc", "#3989d1", "#5c77bc"]) {
      var n;
